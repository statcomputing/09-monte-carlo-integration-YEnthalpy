...
# estimate true E(pg) as mean of 5000 MC result
pg.t <- do1rep.mc(5000, 100, k, sigma, t.end)["PG"]
do1rep.vc <- function(n.path, n.sample, k, sigma, t.end, con.t) {
  # get 100 MC results to calculate the cov and var
  sim.1 <- replicate(100, do1rep.mc(n.path, n.sample, k, sigma, t.end))
  b <- cov(sim.1["PA", ], sim.1["PG", ]) / var(sim.1["PG", ])
  # use the mean of MC as the estimated MC
  mc.est <- rowMeans(sim.1)
  pa.vc <- mc.est["PA"] - b * (mc.est["PG"] - pg.t)
  return(pa.vc)
}
# get 100 result of MC.vc for pa
sim.vc <- replicate(100, do1rep.vc(100, 10, k, sigma, t.end))
sd.vc <- sqrt(var(sim.vc))
# get 100 result of MC for pa
sim.mc <- replicate(100, do1rep.mc(100, 10, k, sigma, t.end))["PA"]
sd.mc <- sqrt(var(sim.mc))
cat("The SD for standard MC is: ", sd.mc, ".\n",
